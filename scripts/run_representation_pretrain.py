...
from grpo_mario_theory.representation_pretrain import run_representation_pretrain
...
def run(config: str, seed: int, profile: bool | str, outdir: str, device: str = "cpu", env_backend: str | None = None):
    ...
    s = section(cfg, "representation", profile)
    psec = section(cfg, "platformer", profile)
    hist, head, targets = run_representation_pretrain(
        ...
        int(s["head_epochs"]),
        int(s["head_eval_every"]),
        s.get("logit_scales", [1.0]),
        resolve_device(device),
    )
    plot_warmstart(head, paths["figures"])
    print(targets.to_string(index=False))
    return hist, head, targets
